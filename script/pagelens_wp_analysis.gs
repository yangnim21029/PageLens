...
var PageLensHtmlTagAudit = (function () {
  ...
  function callPageLensAnalyze(params) {
    ...
    Logger.log(
      `呼叫 analyze，payload 摘要: url=${url}, language=${normalizedLanguage}, focusKeyword=${
        resolvedFocusKeyword || '未提供'
...
<body>
  ...
  function getSheetContext() {
    ...
    // 除錯：顯示所有表頭
    Logger.log(`所有表頭: ${headers.map((h, i) => `${i}:${h}`).join(', ')}`);
    Logger.log(`尋找表頭: passHeader='${CONFIG.passHeader}', suggestionHeader='${CONFIG.suggestionHeader}'`);
