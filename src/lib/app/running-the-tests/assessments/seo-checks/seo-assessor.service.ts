...
export class SEOAssessor {
  ...
  private async checkHeadingStructure(parsedContent: ParsedContent, ingredients: PageIngredients): Promise<AssessmentResult[]> {
    const assessments: AssessmentResult[] = [];
  ...
  private async checkImageOptimization(parsedContent: ParsedContent): Promise<AssessmentResult[]> {
    const assessments: AssessmentResult[] = [];
    ...
    if (images.length === 0) {
      ...
      const status = imagesWithoutAlt.length === images.length ? AssessmentStatus.BAD : AssessmentStatus.OK;
  ...
  private async checkMetaElements(parsedContent: ParsedContent, ingredients: PageIngredients): Promise<AssessmentResult[]> {
    const assessments: AssessmentResult[] = [];
  ...
  private async checkContentLength(parsedContent: ParsedContent): Promise<AssessmentResult[]> {
    const assessments: AssessmentResult[] = [];
   ...
   * 語言感知的單詞計算
   */
  private countWordsLanguageAware(text: string): number {
   ...
   * 取得標題長度標準（根據語言調整）
   */
  private getTitleLengthStandards(title: string): { minLength: number; maxLength: number } {
    const chineseCharRegex = /[\u4e00-\u9fff]/g;
    ...
    if (chineseRatio > 0.5) {
      // 中文為主：字符數標準需要調整（中文字符顯示寬度約為英文的2倍）
  ...
  /**
   * 取得 Meta Description 長度標準（根據語言調整）
   */
  private getMetaDescriptionLengthStandards(description: string): { minLength: number; maxLength: number } {
    const chineseCharRegex = /[\u4e00-\u9fff]/g;
